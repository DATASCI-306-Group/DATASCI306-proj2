...
ggplot(data = NAME_BASICS) + 
  geom_density(aes(x = birthYear), bins = 700, fill = "plum", color = "darkblue", alpha = .7) + coord_cartesian(xlim = c(1800, 2026)) + theme_light() +
  geom_density(aes(x = deathYear), bins = 700, fill = "lightblue", color = "darkblue", alpha = .7) +
...
TITLE_BASICS %>% group_by(two_decades, titleType) %>% summarize(mean_runtime = mean(as.numeric(runtimeMinutes), na.rm = TRUE))
...
NAME_BASICS %>% filter(str_detect(primaryName, regex("^([A-Za-z]+)\\s\\1", ignore_case = TRUE)))
```
