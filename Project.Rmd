...
ggplot(data = TITLE_BASICS_GENRES) + 
  geom_bar(aes(x = releaseYear, fill = genres)) + 
  theme_light() + 
...
TITLE_BASICS_RATINGS |>
  ...
  ggplot() + 
  geom_line(aes(x = releaseYear, y = meanNumVotes)) + 
  theme_light() + 
  labs(x = "Release Year", y = "Average Number of Votes")

TITLE_BASICS_RATINGS |>
  group_by(releaseYear) |>
  summarise(meanNumVotes = mean(numVotes, na.rm = TRUE)) |>
  filter(!is.na(releaseYear)) |>
  cor(x = .$releaseYear, y = .$meanNumVotes)
