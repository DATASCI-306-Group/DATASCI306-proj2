...
observeEvent(input$new_game, {
  ...
  sampled_actors <- sample(non_bacon_actors, size = 5, prob = actor_weights)
  updateRadioButtons(session, "selection", choices = sampled_actors)
})
  ...
  observeEvent(input$choose, {
    ...
    if(game$phase == "actor") {
      ...
      if(chosen_actor == "Kevin Bacon") {
        game$won <- TRUE
        game$active <- FALSE
        updateRadioButtons(session, "selection", choices = c("Game over"))
        return()
      }
      ...
      if ("Kevin Bacon" %in% get_cast(chosen_movie) && !"Kevin Bacon" %in% cast) {
        cast <- c(cast, "Kevin Bacon")
      }
  ...
  output$path_display <- renderUI({
    if(length(game$path) == 0) {
      return(tags$p(tags$em("No moves yet.")))
    }
    ...
    for(i in seq_along(game$path)) {
      if(i %% 2 == 1) {
        combined_path <- c(combined_path, list(tags$strong(game$path[i])))
      } else {
        combined_path <- c(combined_path, list(tags$em(game$path[i])))
      }
      
      if(i < length(game$path)) {
        combined_path <- c(combined_path, list(HTML(" &rarr; ")))
      }
    }
    
    do.call(tagList, combined_path)
  })
  ...
  output$result_message <- renderText({
    if(game$won) {
      paste0("You found Kevin Bacon in ", game$step - 1, " steps!")
    } else if(game$lost) {
